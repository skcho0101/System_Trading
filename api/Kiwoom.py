...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        self._comm_connect()
        self.account_number = self.get_account_number_list()
        self.tr_event_loop = QEventLoop()

        self.order = {}  #종목 코드를 키 값으로 해당 종목의 주문 정보를 담은 딕셔너리
        self.balance = {} #종목 코드를 키값으로 해당 종목의 매수 정보를 담은 딕셔너리

        self.universe_realtime_transaction_info = {}  #실시간 제결 정보를 저장할 딕셔너리 선언



#설치한 api를 사용할 수 있도록 서정
    def _make_kiwoom_instance(self):
        self.setControl("KHOPENAPI.KHOpenAPICtrl.1")

#로그인, 실시간 정보. 기타 정보를 제공받을 수 있는 데이터에 대한 응답을 받을 수 있는 slot함수를 등록 1433
    def _set_signal_slots(self):
        self.OnEventConnect.connect(self._login_slot)
        self.OnReceiveTrData.connect(self._on_receive_tr_data)  #tr의 응답결과를 _on_receive_tr_data를 받도록 설정
        self.OnReceiveMsg.connect(self._on_receive_msg)   #TR/주문 메시지를 _on_receive_msg로 받도록 설정
        self.OnReceiveChejanData.connect(self._on_chejan_slot) #주문 접수/체결 결과를 _on_chejan_slot으로 받도로 설정
...
#로그인 요청을 보냄
    def _comm_connect(self):  #로그인 함수: 로그인 요청 신호를 보낸 이후 응답 대기를 서렁하는 함수
        self.dynamicCall("CommConnect()")

        self.login_event_loop = QEventLoop()  #로그인 시도 결과에 대한 응답 대기 시작
        self.login_event_loop.exec_()
#계좌번호 얻어 오는 함수
    def get_account_number_list(self,tag="ACCNO"):
...
#실시간 시세  등록 함수
    ...
    def _on_receive_tr_data(self,screen_no, rqname, trcode, recode_name,next,unused1, unused2,unused3,unused4):
    ...
    def _on_receive_msg(selfs,screen_no, rqname, trcode, msg):
